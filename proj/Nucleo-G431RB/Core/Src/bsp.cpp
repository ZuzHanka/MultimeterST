...
size_t terminal_receive(char * buff, size_t buff_size)
{
	...
	while (buff_size > 0)
	{
		HAL_StatusTypeDef status = HAL_UART_Receive(&hlpuart1, (uint8_t*) buff, 1, TIMEOUT);
...
{
	return HAL_OK == HAL_UART_Transmit(&hlpuart1, (uint8_t*) buff, buff_size, TIMEOUT * buff_size);
}
...
void adc_init(ADC_TypeDef * adc, const adc_conf_t adc_conf[], size_t chan_count)
{
	...
	{
		hdma_adc.Init.Request = DMA_REQUEST_ADC2;
	}
	else
	{
		Error_Handler();
